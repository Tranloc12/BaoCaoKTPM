...
Repository: Tranloc12/BaoCaoKTPM

C#.NETNUnitSeleniumPostmanJSON Server
...
📊 Bảng Kịch Bản Test Cases Mẫu
🛠️ Hướng Dẫn Cài Đặt Môi Trường
🚀 Hướng Dẫn Thực Thi Kiểm Thử (Run Tests)
...
Kiểm thử đơn vị (Unit Testing): Kiểm thử các hàm/phương thức logic toán học (Tính chu vi, diện tích Hình Chữ Nhật) sử dụng NUnit/MSTest trên nền tảng C# / .NET.
Kiểm thử tự động giao diện (UI Automation Testing): Sử dụng Selenium WebDriver (C#) để giả lập hành vi người dùng, kiểm thử luồng đăng nhập và điều hướng trên trang báo điện tử Tuổi Trẻ.
Kiểm thử API (API Testing): Khởi tạo Mock REST API server bằng JSON Server (Node.js) và xây dựng bộ kịch bản kiểm thử tự động trên Postman (Validate Status Code, Response Schema, Response Time).
...
Test Case ID	Module / Feature	Mô Tả Kịch Bản (Description)	Dữ Liệu Đầu Vào (Input Data)	Kết Quả Mong Đợi (Expected Result)	Trạng Thái
TC1_NUNIT_01	Unit Test (Perimeter)	Kiểm thử tính chu vi với các số dương hợp lệ	Length = 5, Width = 4	Chu vi = 18	PASS
TC2_NUNIT_02	Unit Test (Perimeter)	Kiểm thử tính chu vi với giá trị biên nhỏ nhất	Length = 1, Width = 1	Chu vi = 4	PASS
TC3_NUNIT_03	Unit Test (Perimeter)	Kiểm thử khi nhập kích thước âm	Length = -5, Width = 4	Ném ngoại lệ ArgumentException	PASS
TC1_UI_01	Selenium (Tuoitre.vn)	Đăng nhập để trống thông tin	Username = "", Password = ""	Hiển thị class txt-error-login	PASS
TC2_UI_02	Selenium (Tuoitre.vn)	Tìm kiếm bài viết theo từ khóa	SearchKey = "Kiểm thử phần mềm"	Chuyển hướng đến trang kết quả tìm kiếm	PASS
TC1_API_01	Postman (JSON Server)	Lấy danh sách tài nguyên (GET)	Endpoint: GET /posts	Status 200 OK, Content-Type JSON	PASS
TC2_API_02	Postman (JSON Server)	Thêm mới tài nguyên (POST)	Body: { "title": "KTPM", "author": "Loc" }	Status 201 Created, ID tự sinh	PASS
...
Node.js & npm (Dùng cho API Testing):
...
Postman Desktop:
Tải ứng dụng Postman tại postman.com/downloads.
...
Truy cập thư mục DoAn_KTPM/DoAn_KTPM_30_Loc/KTPM_NUNIT_30_Loc/.
Mở file solution KTPM_NUNIT_30_Loc.sln bằng Visual Studio.
Trên thanh menu, chọn Test ➔ Test Explorer (Hoặc nhấn tổ hợp phím Ctrl + E, T).
Nhấn Run All Tests để thực thi toàn bộ bộ kiểm thử NUnit và Selenium WebDriver.
🔹 Bước 3: Chạy API Testing (JSON Server + Postman)
Mở cửa sổ Terminal/Command Prompt tại thư mục chứa file db.json (DoAn_KTPM/DoAn_KTPM_30_Loc/JsonServer_30_Loc/).
Khởi chạy Mock REST Server:
bash
...
📄 File Báo Cáo Word: 
KTPM_BaoCao_30_Loc.docx
 (Trình bày cơ sở lý thuyết, sơ đồ luồng kiểm thử, chụp màn hình kết quả chạy thực tế).
📊 File Excel Test Cases: 
KTPM_BangTestCase_30_Loc.xlsx
 (Bảng ma trận chi tiết các điều kiện kiểm thử, input, output và status).
👨‍💻 Sinh Viên Thực Hiện
Họ và tên: Trần Lộc
Mã SV / STT: 30
GitHub Profile: @Tranloc12
